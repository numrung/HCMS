...
# --- 4. ฟังก์ชันสร้าง Mail Link (ปรับปรุงข้อความให้สบายตาและเป็นระบบขึ้น) ---
def create_mailto_link(row):
    ...
    body = (
        ...
        f"ระบบ CMS ตรวจพบว่ารถยนต์ในความดูแลของท่าน ถึงกำหนดต้องเข้ารับการบำรุงรักษา ณ ศูนย์บริการ โดยมีรายละเอียดดังนี้ครับ\n\n"
        f"🚗 ข้อมูลยานพาหนะ:\n"
        f"• หมายเลขทะเบียนรถ: {row['ป้ายทะเบียนรถ']}\n"
        f"• เลขไมล์ปัจจุบัน: {row['เลขไมล์สิ้นสุด']:,} กม.\n"
        f"• กำหนดเข้าศูนย์บริการรอบถัดไป: {row['เลขไมล์เข้าศูนย์บริการรอบถัดไป']:,} กม.\n"
        f"• ระยะคงเหลือ: {row['ระยะห่าง']:,} กม.{status_tag}\n\n"
        f"----------------------------------------\n\n"
        f"💡 ข้อแนะนำและการดำเนินการ:\n"
        f"1. กรุณาติดต่อและนัดหมายศูนย์บริการล่วงหน้า เพื่อความสะดวกรวดเร็ว\n"
        f"2. หากมีข้อสงสัยหรือต้องการข้อมูลเพิ่มเติม กรุณาติดต่อฝ่ายทรัพยากรบุคคล ประจำพื้นที่\n\n"
        f"จึงเรียนมาเพื่อทราบและโปรดดำเนินการ\n\n"
        f"ขอแสดงความนับถือ\n"
        f"ระบบแจ้งเตือนการบำรุงรักษารถยนต์ (CMS)\n"
        f"จัดทำโดย: บุคคลและธุรการป้อมนำR4"
    )
        ...
        if not alerts.empty:
            ...
            if st.button("💬 ส่ง LINE สรุปให้แอดมิน", type="primary", use_container_width=True):
                if line_token and line_user_id:
                    ...
                    st.error("❌ ไม่พบข้อมูล Line Token หรือ User ID ในไฟล์ตั้งค่า")
        else:
